...
from pjtk2.models import SamplePoint, Project, ProjectPolygon
...
def find_roi_projects(roi, project_types=None, first_year=None, last_year=None):
    ...
    try:
        if roi.geom_type == "Polygon" or roi.geom_type == "MultiPolygon":
            ...
            # we will use the ProjectMultiPoints to find those project
            # that are contained or overlap our region of interest -
            # without having to loop over all of the sample points and
            # perform expenisve calcution each time. - We can't use
            # the convex hull because pojects with less than three points
            # can't have a polygon.

            projects = {}

            contained = (
                Project.objects.filter(multipoints__geom__within=roi)
            ...
            if project_types:
                contained = contained.filter(project_type__in=project_types)
                overlapping = overlapping.filter(project_type__in=project_types)

            if first_year:
                contained = contained.filter(year__gte=first_year)
                overlapping = overlapping.filter(year__gte=first_year)
            if last_year:
                contained = contained.filter(year__lte=last_year)
                overlapping = overlapping.filter(year__lte=last_year)
...
def find_roi_points(roi, prj_cds):
    ...
    try:
        if roi.geom_type == "Polygon" or roi.geom_type == "MultiPolygon":
            ...
            # the points associated with that project
            for prj in prj_cds:
                samples = SamplePoint.objects.filter(
                    project__prj_cd=prj
                ).prefetch_related("project", "project__project_type")
                aggregate_samples = samples.aggregate(Collect("geom")).get(
                    "geom__collect"
                )
