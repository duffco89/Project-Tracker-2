...
def test_anon_user_cannot_approve_project(client, project):
    ...
    response = client.get(reverse("approve_project", kwargs={"slug": project.slug}))
...
@pytest.mark.django_db
def test_anon_user_cannot_cancel_project(client, project):
    """An anonymous user should not be able to approve projects.  If they
        try to access the cancel page directly, they should be
        re-directed to the login page and the project will remain
        active and approved.
    ...
    response = client.get(reverse("cancel_project", kwargs={"slug": project.slug}))
    # follow=True)
    assert client.login() == False
...
def test_anon_user_cannot_signoff_project(client, project):
    ...
    response = client.get(reverse("signoff_project", kwargs={"slug": project.slug}))
    # verify that we're redirected to login page
    assert response.status_code == 302
    url = reverse("login").strip("/")
    assert url in response["Location"]
