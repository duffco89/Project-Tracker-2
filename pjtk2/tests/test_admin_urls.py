...
class TestUrls(DemoTestCase):
    ...
    def test_urls(self):
        ...
        URLS = (
            ...
            (reverse("TaggedProjects", args=(self.tag,)), {"status_code": 200}),
            (reverse("project_tag_list"), {"status_code": 200}),
            (reverse("find_projects_roi"), {"status_code": 200}),
            (reverse("about_view"), {"status_code": 200}),
            (reverse("report_desc_view"), {"status_code": 200}),
            # the remaining views all require login:
            (reverse("NewProject"), {"status_code": 302}),
            (reverse("CopyProject", args=(self.project.slug,)), {"status_code": 302}),
            (reverse("EditProject", args=(self.project.slug,)), {"status_code": 302}),
            (reverse("ApproveProjects"), {"status_code": 302}),
            (
                reverse("signoff_project", args=(self.project.slug,)),
                {"status_code": 302},
            ),
            (reverse("MyProjects"), {"status_code": 302}),
            (
                reverse(
                    "EmployeeProjects", kwargs={"employee_name": self.user.username}
                ),
                {"status_code": 302},
            ),
            (
            ...
            (reverse("ReportUpload", args=(self.project.slug,)), {"status_code": 302}),
