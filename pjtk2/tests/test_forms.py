...
class TestApproveProjectForm(TestCase):
    def setUp(self):
        ...
        self.proj = ProjectFactory.create(
            prj_cd="LHA_IA12_123", prj_ldr=self.user, prj_nm="Homer's Odyssey"
        )

    def test_approve_projects_form(self):
    ...
    def test_approve_projects_form2(self):
...
class TestProjectForm(TestCase):
    ...
    def test_uppercase_tag_data(self):
        """
        Rags should be automatically converted to lower case. We
...
class TestSelectSistersForm(TestCase):
    ...
    def test_initial_values(self):
        ...
        initial = dict(
            prj_cd="ZZZ_ZZ12_ZZZ", prj_nm="The Wrong Project", prj_ldr="George Costanza"
        )
...
class TestReportUploadForm(TestCase):
    def setUp(self):
        ...
        self.rep1 = MilestoneFactory.create(
            label="Summary Report", category="Core", order=1, shared=True, report=True
        )
    ...
    def test_submit_reports(self):
        ...
        file_data = {
            "report_path": SimpleUploadedFile(self.mock_file_name, self.mock_file)
        }
        ...
        filepath = os.path.join(
            UPLOAD_TO, os.path.split(self.mock_file_name)[1]
        ).replace("\\", "/")
    ...
    def test_upload_with_existing_report(self):
        ...
        # replace the first.
        file_data = {
            "report_path": SimpleUploadedFile(self.mock_file2_name, self.mock_file2)
        }
        form = ReportUploadForm(
            initial=initial,
            data=data,
        ...
        filepath = os.path.join(
            UPLOAD_TO, os.path.split(self.mock_file2_name)[1]
        ).replace("\\", "/")
    ...
    def tearDown(self):
        ...
        # this test
        filepath = os.path.join(
            settings.MEDIA_ROOT, UPLOAD_TO, os.path.split(self.mock_file_name)[1]
        )
        try:
            os.remove(filepath)
        except:
        ...
        filepath = os.path.join(
            settings.MEDIA_ROOT, UPLOAD_TO, os.path.split(self.mock_file2_name)[1]
        )
        try:
            os.remove(filepath)
        except:
