...
def test_image_urls_redirect_to_login(client, project, user):
    ...
    urls = [['project_add_image', {'slug':project.slug}],
...
def test_image_urls_joe_user_redirect_to_details(client, project, joe_user):
    ...
    for url_name,kwargs in urls:
        ...
        assert 'pjtk2/projectdetail.html'in [t.name
...
def test_nonexistant_image_redirect_to_404(client, project, user):
    ...
    urls = [['project_add_image', {'slug':'zzz_zz99_zzz'}],
...
def test_add_image_get_canedit(client, project, user, manager, dba):
    ...
    for current_user in users:
        ...
        login = client.login(username=current_user.username,
                             password='Abcd1234')
        assert login is True
        response = client.get(reverse('project_add_image',
        ...
        content = response.content.decode("utf-8")
        shouldbe = "<h4>Add an image or graph for {}</h4>".\
                   format(project.prj_nm)
...
def test_add_image_post_save_canedit(client, project, user, manager, dba):
    ...
    for i, current_user in enumerate(users):
        ...
        img = SimpleUploadedFile('myimage{}.jpg'.format(i),
...
def test_edit_image_get_canedit(client, project, user, manager, dba):
    ...
    for current_user in users:
        ...
        login = client.login(username=current_user.username,
                             password='Abcd1234')
        assert login is True
        response = client.get(reverse('project_edit_image',
                                      kwargs={'pk':image.id}))
...
def test_edit_image_post_next_canedit(client, project, user, manager, dba):
    ...
    for i, current_user in enumerate(users):
        ...
        assert response.status_code == 200
        assert 'pjtk2/project_sort_images.html'in [t.name
                                              for t in response.templates]
        content = response.content.decode("utf-8")
...
def test_edit_image_max_report_cutoff(client, project, user):
    ...
    for img in range(max_images+1):
        ...
        ProjectImageFactory(project=project, order=img,
                                 caption=caption,
                            report=True)
    ...
    response = client.post(url, data = image_data,
                           follow=True)
...
def test_delete_image_get_canedit(client, project, user, manager, dba):
    ...
    for current_user in users:
        ...
        login = client.login(username=current_user.username,
                             password='Abcd1234')
        assert login is True
        response = client.get(reverse('delete_image_file',
                                      kwargs={'pk':image.id}))
...
def test_delete_image_post_canedit(client, project, user, manager, dba):
    ...
    for current_user in users:
        ...
        login = client.login(username=current_user.username,
                             password='Abcd1234')
        assert login is True
        response = client.post(reverse('delete_image_file',
                                       kwargs={'pk':image.id}),
                               follow=True)
...
def test_list_project_image_canedit(client, project, user, manager, dba):
    ...
    #some images:
    captions = ["The first image",
                "The second image",
                "The third image"]

    for i, caption in enumerate(captions):
        ProjectImageFactory(project=project, order=i,
                                 caption=caption)
    ...
    for current_user in users:
        ...
        login = client.login(username=current_user.username,
                             password='Abcd1234')
        assert login is True
        response = client.post(reverse('project_images',
                                       kwargs={'slug':project.slug}),
                               follow=True)
